...
HitEffect::HitEffect(Node* node) :
		hitType(R::CharacterType::NONAME), safeToRemove(false) {
	this->node = node;
}
void HitEffect::setHitStyle(R::CharacterType type) {
	this->hitType = type;
}
...
void HitEffect::start() {
	...
	spine::SkeletonAnimation* animation =
			spine::SkeletonAnimation::createWithFile("spine/Trung don.json",
					"spine/Trung don.atlas", 1);
	...
	animation->setCompleteListener(
			[this, animation](int trackIndex, int loopCount) {
				dismiss();
			});
...
void KameKameHa::update(artemis::World* world) {
	...
	if (velocity != Vec2::ZERO) {
		...
		node->setPositionX(
				node->getPositionX() + direction * velocity.x * delta);
		node->setPositionY(
				node->getPositionY() + direction * velocity.y * delta);
	}
	...
	if (state == STATE_FLYING) {
		...
		PosComponent* targetPositon = (PosComponent*) e.getComponent<
				PosComponent>();
		BoundComponent* targetBound = (BoundComponent*) e.getComponent<
				BoundComponent>();
		if (targetBound && targetPositon) {
			Rect targetRect = Rect(targetPositon->x + targetBound->x1,
					targetPositon->y + targetBound->y1, targetBound->getWidth(),
					targetBound->getHeight());
			if (targetRect.intersectsCircle(node->getPosition(), 10)) {
				StateComponent* targetState = (StateComponent*) e.getComponent<
						StateComponent>();
				CharacterInfoComponent* targetInfo =
						(CharacterInfoComponent*) e.getComponent<
								CharacterInfoComponent>();
				targetInfo->blood -= powerOfAttack;
				targetState->direction =
						(node->getPositionX() - 10
								< targetPositon->x + targetBound->x1) ?
								R::Direction::RIGHT : R::Direction::LEFT;
	...
	if (state == STATE_FLYING
			&& (node->getPositionX() <= 0
					|| node->getPositionX() >= R::Constants::MAX_SCREEN_WIDTH)) {
		bienMat();
	}
}

NotEnoughManaEffect::NotEnoughManaEffect(Node* node) {
	this->node = node;
}
...
void NotEnoughManaEffect::start() {
	...
	ui::Text* text = ui::Text::create("Not enough mana",
	...
	text->runAction(
			Sequence::create(scaleOut, timeLine, CallFunc::create([=]() {
				dismiss();
			}), nullptr));
...
IntroduceMessage::IntroduceMessage(Node* node) {
	this->node = node;
}
...
void IntroduceMessage::start(std::string message) {
	...
	text->runAction(
			RepeatForever::create(Sequence::create(fadeIn, fadeOut, nullptr)));
	text->setPosition(Vec2(0, 0));
	node->addChild(text);
}

PlayerInfoLeft::PlayerInfoLeft(Node* container, std::string name,
		std::string avatar) :
		_name(name), _avatar(avatar) {
	this->node = container;
	...
	imgAvatar->setPosition(
			Vec2(imgAvatar->getContentSize().width / 2,
					imgAvatar->getContentSize().height / 2 + 20));
	board_bg->setPosition(
			Vec2(
					imgAvatar->getContentSize().width
							+ board_bg->getContentSize().width / 2 - 20,
					board_bg->getContentSize().height / 2));
	playerName->setAnchorPoint(Vec2(0, .5f));
	playerName->setPosition(
			Vec2(
					board_bg->getPositionX()
							- board_bg->getContentSize().width / 2 + 30,
					board_bg->getPositionY() + 10));

	bgMana->setAnchorPoint(Vec2(0, .5f));
	bgMana->setPosition(
			Vec2(playerName->getPositionX(),
					playerName->getPositionY()
							- playerName->getContentSize().height / 2
							- bgMana->getContentSize().height / 2));
	...
	bgBlood->setPosition(
			Vec2(playerName->getPositionX() - 8,
					bgMana->getPositionY() - bgMana->getContentSize().height / 2
							- bgMana->getContentSize().height / 2 - 6));
...
PlayerInfoRight::PlayerInfoRight(Node* container, std::string name,
		std::string avatar) :
		_name(name), _avatar(avatar) {
	this->node = container;
	ui::ImageView* imgAvatar = ui::ImageView::create(_avatar);
	...
	board_bg->setPosition(
			Vec2(board_bg->getContentSize().width / 2,
					board_bg->getContentSize().height / 2));
	imgAvatar->setPosition(
			Vec2(
					board_bg->getPositionX()
							+ board_bg->getContentSize().width / 2 + 10,
					board_bg->getPositionY() + 20));
	playerName->setAnchorPoint(Vec2(0, .5f));
	playerName->setPosition(
			Vec2(board_bg->getPositionX() - 30, board_bg->getPositionY() + 10));

	bgMana->setAnchorPoint(Vec2(1, .5f));
	bgMana->setPosition(
			Vec2(board_bg->getPositionX() + 52,
					playerName->getPositionY()
							- playerName->getContentSize().height / 2
							- bgMana->getContentSize().height / 2));
...
PauseScene::PauseScene(Node* node) :
		isShowing(false) {
	this->node = node;
...
void PauseScene::showPauseScene() {
	...
	btnSound->setPosition(
			Vec2(
					node->getContentSize().width / 2
							- btnSound->getContentSize().width - 20,
					30 + btnSound->getContentSize().height));
	btnMusic->setPosition(
			Vec2(node->getContentSize().width / 2,
					30 + btnSound->getContentSize().height));
	btnGuide->setPosition(
			Vec2(
					node->getContentSize().width / 2
							+ btnGuide->getContentSize().width + 20,
					30 + btnSound->getContentSize().height));

	btnMenu->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnMusic->getPositionY() + btnMusic->getContentSize().height
							+ 30));
	btnReplay->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnMenu->getPositionY() + btnMenu->getContentSize().height
							+ 30));
	btnContinue->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnReplay->getPositionY()
							+ btnReplay->getContentSize().height + 30));
	...
	btnSound->addClickEventListener(
			[=](Ref* sender) {
	...
	btnMusic->addClickEventListener(
			[=](Ref* sender) {
				if (R::Constants::soundEnable) {
	...
	btnContinue->addClickEventListener(
			[=](Ref* sender) {
	...
	btnReplay->addClickEventListener(
			[=](Ref* sender) {
...
void Winscene::showWinScene() {
	...
	btnShare->setPosition(
			Vec2(
					node->getContentSize().width / 2
							- btnShare->getContentSize().width,
					20 + btnShare->getContentSize().height / 2));
	btnRate->setPosition(
			Vec2(
					node->getContentSize().width / 2
							+ btnRate->getContentSize().width,
					20 + btnRate->getContentSize().height / 2));

	btnMenu->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnRate->getPositionY()
							+ btnRate->getContentSize().height / 2 + 30
							+ btnMenu->getContentSize().height / 2));
	btnReplay->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnMenu->getPositionY() + btnMenu->getContentSize().height
							+ 20));
	btnNext->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnReplay->getPositionY()
							+ btnReplay->getContentSize().height + 20));
	text->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnNext->getPositionY()
							+ btnNext->getContentSize().height / 2 + 40
							+ text->getContentSize().height / 2));
...
void LoseScene::showLoseScene() {
	...
	text->setPosition(
			Vec2(node->getContentSize().width / 2,
					btnReplay->getPositionY()
							+ btnReplay->getContentSize().height / 2 + 40
							+ text->getContentSize().height / 2));
...
void RemainingLife::update() {

	for (int i = 0; i < R::Constants::MAX_LIFE; i++) {
		if (i < R::Constants::remaininglife) {
			hearts.at(i)->setColor(Color3B::RED);
		}else{
			hearts.at(i)->setColor(Color3B::WHITE);
		}
	}
}
...
void RemainingLife::updatePosition() {
	...
	float width = text->getContentSize().width
			+ (pading + hearts.at(0)->getContentSize().width) * hearts.size();
	float height = MAX(text->getContentSize().height,
			hearts.at(0)->getContentSize().height);
	CCLOG("%f %f ", text->getContentSize().width,
			hearts.at(0)->getContentSize().width);
	...
	for (int i = 0; i < hearts.size(); i++) {
		...
		heart->setPosition(
				Vec2(
						text->getContentSize().width + (i + 1) * pading
								+ i * heart->getContentSize().width, 0));
	}
}
...
DialogComfirm::DialogComfirm() {
	...
	layout->setContentSize(
			Director::getInstance()->getRunningScene()->getContentSize());
	...
	bg->setBackGroundImage("textures/bgNotify.png",
			ui::Widget::TextureResType::LOCAL);
	...
	positive = ui::Button::create("textures/btnOK.png",
			"textures/btnOK_down.png", "textures/btnOK.png",
			ui::Widget::TextureResType::LOCAL);
	...
	negative = ui::Button::create("textures/btnOK.png",
			"textures/btnOK_down.png", "textures/btnOK.png",
			ui::Widget::TextureResType::LOCAL);
	...
	negative->setContentSize(
			Size(negative->getContentSize().width + 20,
					negative->getContentSize().height));
...
void DialogComfirm::setNegative(std::string text,
		const std::function<void()> & callback) {
	negative->setTitleText(text);
	negativeCallback = callback;
}

void DialogComfirm::setPositive(std::string text,
		const std::function<void()> & callback) {
...
void DialogComfirmInGame::setNegative(std::string text,
		const std::function<void()> & callback) {
	negative->setTitleText(text);
	negativeCallback = callback;
}

void DialogComfirmInGame::setPositive(std::string text,
		const std::function<void()> & callback) {
	positive->setTitleText(text);
	positiveCallback = callback;
}
...
void HowToPlay::showSwipeLeft(const std::function<void()> &callback) {
	...
	arrow->runAction(
			RepeatForever::create(
					Sequence::create(spawn, DelayTime::create(.4f), cal,
							nullptr)));
