...
StateComponent::StateComponent() :jump(false), totalHit(0), totalAttack(0),hitDetected(0),trungdonlientiep(0), doneAction(true), attack(R::Attack::NONE), defense(R::Defense::NONE), characterBase(nullptr), state(R::CharacterState::STAND), time_on_state(1), direction(R::Direction::AUTO){}

void StateComponent::setState(R::CharacterState newState){
	if (newState == R::CharacterState::JUMP){
		jump = true;
	}
	if (newState != R::CharacterState::ATTACK)	{ 
		hitDetected = 0;
	}
	if (newState == R::CharacterState::DEFENSE){
		trungdonlientiep++;
		totalHit++;
	}
	else if (newState == R::CharacterState::ATTACK) {
		trungdonlientiep = 0;
		totalAttack++;
	}
...
SkillComponent::SkillComponent(KameKameHa* kame) : kamekameha(kame){}


SkillComponent::~SkillComponent(){ 
	kamekameha->node->removeFromParent();
	delete kamekameha;
}
